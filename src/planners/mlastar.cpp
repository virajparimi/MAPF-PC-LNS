...
AgentTaskPath MultiLabelSpaceTimeAStar::findPathSegment(
  ...
  allNodesTable_.emplace(
      makeNodeKey(start->location, start->timestep, start->stage,
                  start->waitAtGoal),
      start);
  ...
  while (!openList_.empty()) {
    ...
    auto tryExpandSuccessor = [&](int successor, int nextTimestep) {
      ...
      bool successorWaitAtGoal = false;
      if (currentStage == goalLocations.size() - 1 &&
          successor == goalLocations.back() &&
          current->location == goalLocations.back()) {
        successorWaitAtGoal = true;
      }
      const auto successorKey =
          makeNodeKey(successor, nextTimestep, currentStage,
                      successorWaitAtGoal);

      auto it = allNodesTable_.find(successorKey);
      if (it == allNodesTable_.end()) {
        ...
        next->waitAtGoal = successorWaitAtGoal;
        next->refreshTieBreaker();
        pushNode(next);
        allNodesTable_.emplace(successorKey, next);
        return;
      }
      ...
      candidate.distanceToNext = getStageGoalDistance(currentStage, successor);
      candidate.waitAtGoal = successorWaitAtGoal;
      candidate.refreshTieBreaker();

      if (existing->getFVal() > candidate.getFVal() ||
          (existing->getFVal() == candidate.getFVal() &&
           LLNode::FocalCompareNode()(existing, &candidate))) {
        // allNodesTable_ hashes by (location,timestep,stage,waitAtGoal).
        // Keep this key invariant unchanged for in-place updates.
        assert(existing->location == candidate.location);
        assert(existing->timestep == candidate.timestep);
        assert(existing->stage == candidate.stage);
        assert(existing->waitAtGoal == candidate.waitAtGoal);
        candidate.inOpenlist = existing->inOpenlist;
        if (!existing->inOpenlist) {
          ...
          reopened->waitAtGoal = successorWaitAtGoal;
          reopened->refreshTieBreaker();
          allNodesTable_.erase(it);
          allNodesTable_.emplace(successorKey, reopened);
          pushNode(reopened);
        } else {
          bool addToFocal = false, updateInFocal = false, updateOpen = false;
          const int oldFVal = existing->getFVal();
          ...
          existing->overwriteSearchStateFrom(candidate);
          if (oldFVal != newFVal) {
            registerOpenNodeByF(existing);
          }
          if (updateOpen) {
            openList_.increase(existing->openHandle);
          }
          if (addToFocal) {
            if (!existing->inFocal) {
              existing->focalHandle = focalList_.push(existing);
              existing->inFocal = true;
            } else {
              updateInFocal = true;
            }
          }
          if (updateInFocal && existing->inFocal) {
            focalList_.update(existing->focalHandle);
          }
        }
      }
    };
...
void MultiLabelSpaceTimeAStar::printSearchTree() {
  ...
  for (const auto& entry : allNodesTable_) {
    allNodesSorted.push_back(entry.second);
  }
