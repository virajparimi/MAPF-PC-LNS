...
bool LNS::buildConstraintTableCore(
  ...
  ConstraintTableTraceState traceState;
  ...
  for (int ancestorTask = 0; ancestorTask < (int)ancestorsOfTask.size();
    ...
    ConstraintTableAncestorReservation reservation;
    const bool pendingAncestor =
        isPendingCommitState(lnsNeighborhood_, ancestorTask);
    if (!resolveAncestorReservation(ancestorTask, pendingAncestor, reservation)) {
      return false;
    }
    if (reservation.path == nullptr || reservation.path->empty()) {
      PLOGE << "buildConstraintTable: missing path for ancestor task "
            << ancestorTask << "\n";
      return false;
    }
    if (traceCtTask) {
      traceState.ancestorTasks.push_back(ancestorTask);
      if (reservation.usedPreviousSource) {
        traceState.ancestorFromPrevious.push_back(ancestorTask);
      }
      if (reservation.isFinalTask) {
        traceState.ancestorFinal.push_back(ancestorTask);
      }
    }
    ...
    if (traceCtTask && contributorEnd > traceState.lenMinDriverEnd) {
      traceState.lenMinDriverEnd = contributorEnd;
      traceState.lenMinDriverTask = ancestorTask;
    }
  }
  ...
  forEachNonAncestorReservation(
      [&](int otherTask, int agent, const AgentTaskPath& pathRef,
          bool isFinalTask) {
        if (agent == skipAgent) {
          return;
        }
        ...
        if (traceCtTask) {
          if (useSoftForNonAncestors) {
            traceState.nonAncestorSoft.push_back(otherTask);
          } else {
            traceState.nonAncestorHard.push_back(otherTask);
          }
        }
  ...
  if (traceCtTask) {
    PLOGW << "CTDETAIL " << traceTag << " task=" << task
          << " len_min=" << constraintTable.lengthMin
          << " latest_ts=" << constraintTable.latestTimestep
          << " anc_count=" << traceState.ancestorTasks.size()
          << " anc_prev_count=" << traceState.ancestorFromPrevious.size()
          << " anc_final_count=" << traceState.ancestorFinal.size()
          << " nonanc_hard_count=" << traceState.nonAncestorHard.size()
          << " nonanc_soft_count=" << traceState.nonAncestorSoft.size()
          << " len_min_driver_task=" << traceState.lenMinDriverTask
          << " len_min_driver_end=" << traceState.lenMinDriverEnd
          << " anc=" << summarizeIntList(traceState.ancestorTasks)
          << " anc_prev=" << summarizeIntList(traceState.ancestorFromPrevious)
          << " anc_final=" << summarizeIntList(traceState.ancestorFinal)
          << " nonanc_hard=" << summarizeIntList(traceState.nonAncestorHard)
          << " nonanc_soft=" << summarizeIntList(traceState.nonAncestorSoft);
  }
  return true;
}
...
bool LNS::buildConstraintTable(ConstraintTable& constraintTable,
  ...
  const auto resolveAncestorReservation =
      [&](int ancestorTask, bool pendingAncestor,
          ConstraintTableAncestorReservation& out) -> bool {
    ...
    if (canUseWorkspacePath) {
      out.agent = ancestorTaskAgent;
      out.path = &workspace.taskPaths(ancestorTaskAgent)[(*posLookup)[ancestorTask]];
      out.usedPreviousSource = false;
      out.isFinalTask = (ancestorTask >= 0 && ancestorTask < taskCount)
    ...
    if (pendingAncestor) {
      ...
      out.agent = prevAssignedAgent;
      out.path = &previousSolution_.agents[prevAssignedAgent].taskPaths[prevLocalIndex];
      out.usedPreviousSource = true;
      out.isFinalTask = (ancestorTask >= 0 && ancestorTask < taskCount)
                            ? previousFinalTasks[ancestorTask]
                            : false;
      return true;
    }
  ...
  const auto forEachNonAncestorReservation =
      [&](const std::function<void(int, int, const AgentTaskPath&, bool)>&
              visitor) {
        for (int agent = 0; agent < instance_.getAgentNum(); agent++) {
          const auto& assignments = workspace.assignments(agent);
          ...
          for (int localTask = 0; localTask < localCount; localTask++) {
            const int queuedTask = assignments[localTask];
            if (queuedTask < 0 || queuedTask >= taskCount) {
              continue;
            }
            if ((*ownerLookup)[queuedTask] != agent ||
                (*posLookup)[queuedTask] != localTask) {
              continue;
            }
            ...
            visitor(queuedTask, agent, pathRef, isFinalTask);
          }
        }
      };

  string traceTag = "eval";
  if (findingNextTask) {
    traceTag += "_next";
  }
  traceTag += " agent=" + std::to_string(taskPacket.agent);
  return buildConstraintTableCore(constraintTable, taskPacket.task,
                                  taskPacket.agent, ancestors, traceCtTask,
                                  traceTag, resolveAncestorReservation,
                                  forEachNonAncestorReservation);
}
...
bool LNS::buildConstraintTable(
  ...
  const auto resolveAncestorReservation =
    ...
    if (hasCurrentOwner) {
      ...
      if (mappedCurrentAgent != UNASSIGNED && mappedCurrentAgent >= 0 &&
        ...
        const int mappedCurrentPos =
            (ancestorTask < taskCount &&
             taskOwner[ancestorTask] == mappedCurrentAgent)
                ? taskToPosition[ancestorTask]
                : UNASSIGNED;
      ...
      if (hasPreviousOwner) {
        ...
        if (mappedPreviousAgent != UNASSIGNED && mappedPreviousAgent >= 0 &&
          ...
          const int mappedPreviousPos =
              (ancestorTask < taskCount &&
               previousTaskOwner[ancestorTask] == mappedPreviousAgent)
                  ? previousTaskToPosition[ancestorTask]
                  : UNASSIGNED;
    ...
    out.agent = ancestorTaskAgent;
    out.path = &pathRef;
    out.usedPreviousSource = usingPreviousPath;
    out.isFinalTask = (ancestorTask >= 0 && ancestorTask < taskCount) &&
        ...
        for (int agent = 0; agent < instance_.getAgentNum(); agent++) {
          ...
          for (int localTask = 0; localTask < localCount; localTask++) {
            ...
            if (taskOwner[otherTask] != agent ||
                taskToPosition[otherTask] != localTask) {
              continue;
            }
            const auto& pathRef = paths[localTask];
            if (pathRef.empty()) {
            ...
            const bool isFinalTask = (localTask + 1 == (int)assignments.size());
            visitor(otherTask, agent, pathRef, isFinalTask);
          }
        }
      };

  return buildConstraintTableCore(constraintTable, task, taskAgent, ancestors,
                                  traceCtTask, "commit",
                                  resolveAncestorReservation,
                                  forEachNonAncestorReservation);
}
