...
void reconstructPath(const SIPPSNode* goal, int goalArrivalTime,
                     int startTime, int goalLocation, AgentTaskPath& outPath) {
  if (goal == nullptr) {
  ...
  // Match MLA* semantics: mark the first arrival to the current stage goal.
  for (int i = 0; i < (int)outPath.size(); i++) {
    if (outPath[i].location == goalLocation) {
      outPath[i].isGoal = true;
      break;
    }
  }
}
